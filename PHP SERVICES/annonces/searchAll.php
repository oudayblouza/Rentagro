...
$req = "select a.id id, a.nature nature, a.etat etat, a.titre titre, a.description description, a.prix prix, a.validite validite, a.dateDebut dateDebut, a.duree duree, a.numTel numTel, a.user_id user_id, a.type_id type_id, a.imagesdir imagesdir, a.nbrimages nbrimages, a.delegation_id delegation_id,a.renttype renttype,a.longitude longitude,a.lattitude lattitude, t.id typeID, t.categorie_id cat FROM annonce a, type t WHERE a.type_id= t.id AND t.categorie_id='".$category."' AND etat = 1 AND(titre like '%".$search."%' or nature like '%".$search."%' or description like '%".$search."%')" ;
